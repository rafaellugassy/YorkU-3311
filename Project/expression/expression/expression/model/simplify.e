...
feature
	...
	local
		simplified_set : LINKED_LIST[SIMPLIFY]
		op : BINARY_OP
	do
		create simplified_set.make
		create op.make_binary_op (VOID, "=")
		across s.children as cursor
		loop
			simplified_set.force (create {SIMPLIFY}.make)
		...
		elseif across simplified_set as cursor all cursor.item.value.type ~ simplified_set.at(1).value.type end then
			...
			m.set_error ("Expression is not type-correct")
			error := true
		end
	end
